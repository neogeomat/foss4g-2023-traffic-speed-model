...
def convert_utc_to_local(twitter_df: pd.DataFrame, timezone_name: str):
    ...
    twitter_df['timestamp'] = pd.to_datetime(twitter_df['createdAT'].map(lambda x: x['$date']))
    twitter_df["timestamp_local"] = twitter_df["timestamp"].dt.tz_convert(
        local_timezone
    )
    twitter_df["hour"] = (
        twitter_df["timestamp"].map(lambda x: x.hour).astype("int")
    )
    return twitter_df
...
def preprocess_twitter(filepaths, twitter_dir, timezone_name, aoi_name):
    ...
    # read twitter data
    twitter_file = Path(twitter_dir) / f"{aoi_name}-2018-01-01T00-00-00Z_2020-03-31T23-59-59Z.json"
    ...
    twitter_df.drop(['coords', 'createdAT', 'timestamp', 'timestamp_local'], axis=1, inplace=True)
    ...
    for i, r in enumerate(resolutions):
        ...
        edges_twitter = gpd.sjoin(edges_buffered, twitter_df, how="left").drop(['geometry', 'index_right'], axis=1)
        ...
        #tweet_features = twitter_hour.join(twitter_all, how="left")
...
if __name__ == "__main__":
    aoi_dir = "/Users/cludwig/Development/sm2t/sm2t-traffic-speed-model/data/results/nairobi"
    twitter_dir = "/Users/cludwig/Data/sds/sd17f001/SM2T/COPY-TO-GLOBUS/tweet-data/2018-01-01T00-00-00Z_2020-03-31T23-59-59Z"
    timezone_name = "Afrikca/Nairobi"
    ...
    name = args.name
    """
    preprocess_twitter(aoi_dir, twitter_dir, timezone_name=timezone_name, aoi_name='')
